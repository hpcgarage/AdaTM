  ...
  struct argp_state * state)
{
  ...
  case 'a':
    ...
    } else if(strcmp(arg, "adatm") == 0) {
      args->which[ALG_ADATM] = 1;
    } else {
  ...
  char ** argv)
{
  ...
  args.algerr = NULL;
  args.permerr = NULL;
  args.niters = 5; //5; jli: for debug
  args.nthreads = 1;
  args.scale = 0;
  args.rank = 16; //10; jli: for debug
  args.write = 0;
  args.tile = 0;
  args.permmode = 0;
  args.rtype = PERM_ERROR;
  ...
  if(args.pfname != NULL)  {
    ...
    /* initialize perms */
    opts.perm = perm_alloc(tt->dims, 0);  // jli: don't allocate opts.perm
  }
