...
#define SPLATT_MAX_NMODES ((splatt_idx_t) 88)   // jli: change the maximum modes
...
typedef enum
{
  ...
  SPLATT_ADAPTIVE,  // jli: fresh added
  SPLATT_ADAPTIVE_TIME_EFFICIENT,
  SPLATT_ADAPTIVE_SPACE_EFFICIENT,
} splatt_csf_type;
...
typedef struct
{
  ...
  // jli: fids[0]: NULL (maybe); fptr[nmodes-1]: NULL
 ...
 *****************************************************************************/
/**
* @brief Adaptive semi-sparse tensor sequence in vCSF format, to store the 
*     intermediate tensors of an MTTKRP operation.
*/
typedef struct rcsf_seq_adaptive
{
  /** @brief The beginning index (corresponding the index of factor matrix) of the tensor sequence. */
  splatt_idx_t begin_imten;

  /** @brief The length of vCSF tensor sequence. */
  splatt_idx_t n_imten;

  /** @brief The sequence of stored intermediate semi-sparse tensors in vCSF. */
  splatt_csf * rcsfs;
...
/**
* @brief An MTTKRP group with the "memo_mode" as the recalculated MTTKRP, and
*   store "n_imten" intermediate tensors in an order of "o_opt".
*/
typedef struct group_properties
{
  /** @brief MTTKRP at this mode is calculated from scratch. */
  splatt_idx_t memo_mode;
  /** @brief The begin index of the intermediate tensor sequence. */
  splatt_idx_t begin_imten;
  /** @brief The length of vCSF tensor sequence. */
  splatt_idx_t n_imten;
  /* o_opt stores the optimal order of the tensor tree, which is non-decreasing to the dimension sizes. 
      o_opt[0] = mode; 
      o_opt[1,...,begin_imten-1]: (n_imten-1) intermediate tensors will be stored.
      o_opt[begin_imten]: have to be stored for our memoization algorithm, otherwise no memoization.
      o_opt[begin_imten+1,...,nmodes-1]: these intermediate tensors are not stored. */
  splatt_idx_t * o_opt;
} group_properties;

/**
* @brief A configuration for Nth-MTTKRP sequence (the whole sequence).
*/
typedef struct configurations_adaptive
{
  /* @brief The number of MTTKRP groups. */
  splatt_idx_t n_grp;
  /* @brief An array of group properties, each entry for one group. */
  group_properties * grp_prop;
  /* @brief Predicted storage space. */
  size_t pspace;
  /* @brief Predicted number of products. */
  size_t pops;
  /* @brief Predicted running time of the Nth-MTTKRP sequence. */
  double ptime;
  /* @brief Measured running time of the Nth-MTTKRP sequence. */
  double rtime;
} configurations_adaptive;


/**
* @brief A simple struct for pair data.
*/
typedef struct pair
{
  splatt_idx_t x;
